...
final class CitiesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new CityService();
	}
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new CityDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new City, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new CityDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new CityViewModel)->files($request->allFiles())->withBody($request->all());
