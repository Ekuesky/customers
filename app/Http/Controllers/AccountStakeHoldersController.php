...
use App\Services\AccountStakeHolderService;
use App\Models\AccountStakeHolder;
use App\Http\ViewModels\AccountStakeHolderViewModel;
use App\Dto\AccountStakeHolderDto;
...
final class AccountStakeHoldersController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new AccountStakeHolderService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /account-stake-holders[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new AccountStakeHolderDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new AccountStakeHolder, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new AccountStakeHolderDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	 ...
	 * @Route /POST /account-stake-holders
	 * 
	 * @param Request $request
	 *
	 * @return JsonResponse
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new AccountStakeHolderViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /account-stake-holders/{id}
	 * @Route /PATCH /account-stake-holders/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /account-stake-holders/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
