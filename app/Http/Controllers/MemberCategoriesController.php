...
use App\Services\MemberCategoryService;
use App\Models\MemberCategory;
use App\Http\ViewModels\MemberCategoryViewModel;
...
final class MemberCategoriesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new MemberCategoryService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /member-categories[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new MemberCategoryDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new MemberCategory, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new MemberCategoryDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new MemberCategoryViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /member-categories/{id}
	 * @Route /PATCH /member-categories/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /member-categories/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
