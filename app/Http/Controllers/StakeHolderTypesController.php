...
use App\Services\StakeHolderTypeService;
use App\Models\StakeHolderType;
use App\Http\ViewModels\StakeHolderTypeViewModel;
use App\Dto\StakeHolderTypeDto;
...
final class StakeHolderTypesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new StakeHolderTypeService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /stake-holder-types[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new StakeHolderTypeDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new StakeHolderType, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new StakeHolderTypeDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new StakeHolderTypeViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /stake-holder-types/{id}
	 * @Route /PATCH /stake-holder-types/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /stake-holder-types/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
