...
use App\Services\MembershipStatusChangeService;
use App\Models\MembershipStatusChange;
use App\Http\ViewModels\MembershipStatusChangeViewModel;
use App\Dto\MembershipStatusChangeDto;
...
final class MembershipStatusChangesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new MembershipStatusChangeService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /membership-status-changes[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new MembershipStatusChangeDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new MembershipStatusChange, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new MembershipStatusChangeDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	 ...
	 * @Route /POST /membership-status-changes
	 * 
	 * @param Request $request
	 *
	 * @return JsonResponse
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new MembershipStatusChangeViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /membership-status-changes/{id}
	 * @Route /PATCH /membership-status-changes/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /membership-status-changes/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
