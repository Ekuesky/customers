...
use App\Services\ActivitySectorService;
use App\Models\ActivitySector;
use App\Http\ViewModels\ActivitySectorViewModel;
use App\Dto\ActivitySectorDto;
...
final class ActivitySectorsController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new ActivitySectorService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /activity-sectors[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new ActivitySectorDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new ActivitySector, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new ActivitySectorDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new ActivitySectorViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /activity-sectors/{id}
	 * @Route /PATCH /activity-sectors/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /activity-sectors/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
