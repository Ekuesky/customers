...
use App\Http\ViewModels\MembershipStatusViewModel;
use App\Dto\MembershipStatusDto;
...
final class MembershipStatusesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new MembershipStatusService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /membership-statuses[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new MembershipStatusDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new MembershipStatus, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new MembershipStatusDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	 ...
	 * @Route /POST /membership-statuses
	 * 
	 * @param Request $request
	 *
	 * @return JsonResponse
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new MembershipStatusViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /membership-statuses/{id}
	 * @Route /PATCH /membership-statuses/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /membership-statuses/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
