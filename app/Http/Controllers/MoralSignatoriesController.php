...
use App\Services\MoralSignatoryService;
use App\Models\MoralSignatory;
use App\Http\ViewModels\MoralSignatoryViewModel;
use App\Dto\MoralSignatoryDto;
...
final class MoralSignatoriesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new MoralSignatoryService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /moral-signatories[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new MoralSignatoryDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new MoralSignatory, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new MoralSignatoryDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new MoralSignatoryViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /moral-signatories/{id}
	 * @Route /PATCH /moral-signatories/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /moral-signatories/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
