...
use App\Services\MoralDocumentService;
use App\Models\MoralDocument;
use App\Http\ViewModels\MoralDocumentViewModel;
use App\Dto\MoralDocumentDto;
...
final class MoralDocumentsController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new MoralDocumentService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /moral-documents[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new MoralDocumentDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new MoralDocument, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new MoralDocumentDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new MoralDocumentViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /moral-documents/{id}
	 * @Route /PATCH /moral-documents/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /moral-documents/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
