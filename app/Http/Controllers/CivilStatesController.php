...
use App\Http\ViewModels\CivilStateViewModel;
use App\Dto\CivilStateDto;
...
final class CivilStatesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new CivilStateService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /civil-states[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new CivilStateDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new CivilState, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new CivilStateDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new CivilStateViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /civil-states/{id}
	 * @Route /PATCH /civil-states/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
