...
use App\Services\DocumentsTypeService;
use App\Models\DocumentsType;
use App\Http\ViewModels\DocumentsTypeViewModel;
use App\Dto\DocumentsTypeDto;
...
final class DocumentsTypesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new DocumentsTypeService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /documents-types[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new DocumentsTypeDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new DocumentsType, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new DocumentsTypeDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new DocumentsTypeViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /documents-types/{id}
	 * @Route /PATCH /documents-types/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
	 ...
	 * @Route /DELETE /documents-types/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
