...
use App\Http\ViewModels\MemberTypeViewModel;
use App\Dto\MemberTypeDto;
...
final class MemberTypesController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new MemberTypeService();
	}

	/**
	 * Display or Returns a list of items
	 * @Route /GET /member-types[/{$id}]
	 * 
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new MemberTypeDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new MemberType, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new MemberTypeDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new MemberTypeViewModel)->files($request->allFiles())->withBody($request->all());
	 ...
	 * @Route /PUT /member-types/{id}
	 * @Route /PATCH /member-types/{id}
	 * 
	 * @param Request $request
	 * @param mixed $id
	 *
