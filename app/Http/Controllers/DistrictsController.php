...
use App\Http\ViewModels\DistrictViewModel;
...
final class DistrictsController
{
	...
	public function __construct(Validator $validator, IActionResponseHandler $response, ActionHandler $service = null)
	{
		...
		$this->service = $service ?? new DistrictService();
	}
	...
	public function index(Request $request, string $id = null)
	{
		...
		$tranformFunc_ = function( $items) {
			return map_query_result($items, function ($value) {
				return $value ? (new DistrictDto)->withModel($value) : $value;
			});
		};
		$filters = drewlabs_databse_parse_client_request_query(new District, $request);
	...
	public function show(Request $request, $id)
	{
		...
		$result = $this->service->handle(Action([
			...
			return null !== $value ? new DistrictDto($value->toArray()) : $value;
		});
		return $this->response->ok($result);
	}
	...
	public function store(Request $request)
	{
		...
		$viewModel_ = (new DistrictViewModel)->files($request->allFiles())->withBody($request->all());
